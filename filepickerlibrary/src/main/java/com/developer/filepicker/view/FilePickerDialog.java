...
import com.developer.filepicker.controller.FileListAdapter;
...
public class FilePickerDialog extends Dialog implements AdapterView.OnItemClickListener {
    ...
    private void dir() {
        ...
        if (properties.offset.isDirectory() && validateOffsetPath()) {
            ...
            parent.setLocation(Objects.requireNonNull(currLoc.getParentFile()).getAbsolutePath());
    ...
    public void onBackPressed() {
        ...
        if (internalList.size() > 0) {
            ...
            File currLoc = new File(fitem.getLocation());
            if (currentDirName.equals(properties.root.getName()) || !currLoc.canRead()) {
                super.onBackPressed();
            } else {
                dname.setText(currLoc.getName());
                dir_path.setText(currLoc.getAbsolutePath());
    ...
    private boolean arePermissionsGranted() {
        int sdkVersion = Build.VERSION.SDK_INT;
        if (sdkVersion >= Build.VERSION_CODES.TIRAMISU) {
            return hasPermission(READ_MEDIA_AUDIO) && hasPermission(READ_MEDIA_VIDEO);
        } else if (sdkVersion >= Build.VERSION_CODES.R) {
...
//        super.onRequestPermissionsResult(requestCode, permissions, grantResults);
        if (requestCode == PERMISSION_REQUEST_CODE) {
            if (grantResults.length > 0 && allPermissionsGranted(grantResults)) {
                Toast.makeText(context, "Permissions grant", Toast.LENGTH_SHORT).show();
            } else {
